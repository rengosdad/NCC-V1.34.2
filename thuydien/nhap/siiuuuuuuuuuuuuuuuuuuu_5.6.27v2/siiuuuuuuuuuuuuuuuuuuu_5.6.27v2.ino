...
//Khởi động LCD
 ...
 {
  {
    Pr=scale.get_units(10);
  Serial.print("one reading:\t");
  Serial.print(scale.get_units(), 1);
  Serial.print("\t| average:\t");
  Serial.println(Pr/100);//((scale.get_units(20), 1)/100);
  //lcd.setCursor(0,1);
 // lcd.print("Pr= ");
 // lcd.setCursor(4,1);
...
//  Serial.print("Hiện tại: "); Serial.println(hientai);
 ...
 for(int i = 0; i < 1000; i++) {
     bawn = average =  average + (.0264 * analogRead(A0) -13.453);//for the 5A mode, 13.51 
...
bt.print(L);//lưu lượng send distance to MIT App
 bt.print(";");
  bt.print(Pr/100); //áp xuất send distance to MIT App 
 bt.print(";");
